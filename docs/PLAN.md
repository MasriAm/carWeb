...
- [ ] **1 — Design tokens and primitives.** Semantic tokens, type scale,
...
- [ ] **2 — Rendering and data architecture.** `src/lib/data` with `server-only`
...
- [ ] **3 — Filters and search.** Full-text search, multi-select facets,
  mileage, agency import, spec origin, dealer, include-sold, log-scale price
  slider, count preview, no-results recovery.
- [ ] **4 — Card and grid.** One link target, exact price, controls outside the
  link, image fallback.
- [ ] **5 — Landing page.** Hero with search, real inventory, browse by body
  type and budget, real counts.
- [ ] **6 — Detail page.** Gallery with lightbox and video, price context,
  similar cars, share, structured data.
- [ ] **7 — Dealer pages and site SEO.** `/dealers`, sitemap, robots.
- [ ] **8 — Account features.** Saved searches, working profile form.
- [ ] **9 — Dealer tooling polish.** Spec origin in the listing form, honest
  verified state.
